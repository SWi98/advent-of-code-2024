    ...
    while i < len(content):
        ...
        if not good_sequence:
            ...
            while j < n:
                ...
                current = current_sequence[j]
                new_sequence = current_sequence.copy()
                if j > 0:
                    previous = current_sequence[j - 1]
                    numbers_ordering[previous] = NumberOrder.BEFORE_CURRENT
                for num_has_to_be_before in rules[current]:
                    if not (
                        (numbers_ordering[num_has_to_be_before] == NumberOrder.NOT_EXIST)
                        ...
                        new_sequence.pop(j)
                        bad_element_idx = new_sequence.index(num_has_to_be_before)
                        new_sequence.insert(bad_element_idx + 1, current)
                        j = -1
                        current_sequence = new_sequence
                        break
                j += 1
            middle_element_idx = int((len(current_sequence)) / 2)
            res += current_sequence[middle_element_idx]
